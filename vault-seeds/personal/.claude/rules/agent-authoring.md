...
The **baseline** - the persistence rule, the agent-memory rule, the concurrency rule, this
rule, and anything governing sync, secrets, or access. Propose changes to those as a
note tagged `#agent/proposal` - anywhere in the vault, folders carry no semantics - and
stop; the owner ratifies. The line: agents extend the system freely, but do not amend
its foundations on their own.
...
house style; scope vault-wide with `**/*.md` unless the rule is genuinely about one
area - folders carry no semantics, so a folder-scoped rule silently misses notes it
should govern (the sync tool quotes the generated `paths:` value, which is what keeps a
leading `*` valid YAML); keep it as short as the obligation allows; and when a rule's subject
