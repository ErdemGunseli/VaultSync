...
description: >-
  ...
  "/use <owner/repo> [skill]", "load the <name> skill from <repo>", "use QUANTSOC's
  render-ops here", or asks what skills another repo offers. Optionally adopt a used skill
  permanently into this repo.
---

# use - use another repository's skills here
...
## `/use <owner/repo>` - list what a repo offers
...
```bash
git clone --depth 1 --filter=blob:none --sparse https://github.com/<owner>/<repo> /tmp/use-<repo>
git -C /tmp/use-<repo> sparse-checkout set .cursor/skills .claude/skills
```

## `/use <owner/repo> <skill>` - load and follow it
...
## `/use <owner/repo> <skill> adopt` - keep it permanently
...
- A used skill is **instructions written for another repo**. Read it critically: skip any
  step that contradicts this repo's own rules (this repo's rules always win), and say so
...
- Using **rules** works the same way (`.cursor/rules/*.mdc` paths), but a used rule is
  context for THIS session only - adopting a rule permanently follows the target repo's own
