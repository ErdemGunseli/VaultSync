...
Ratified boundary (owner, 2026-08-17): this vault is the **human thinking surface** -
vision, backlogs, strategy, shared docs, research written for humans, ideas and their
states. Anything a person creates or interacts with *as planning* lives here as the
source of truth, and agents reference it from here. Code repositories are the machine
execution surface: code, records rule-bound to code (ledgers, canonical docs), and
AI-written implementation plans, whose audience is the executing agent. A document is
never dual-homed - one side is the source, the other at most a pointer or generated
mirror. When an implementation plan graduates to something the owner works with as
planning, it moves here deliberately and the repo copy becomes the pointer.
